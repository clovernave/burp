...
public class MatchReplaceConfigurationPanel extends JPanel {
    ...
    public List<MatchReplace> getData() {
        ...
        for (int i = 0; i < rowCount; i++) {
            int count = Integer.valueOf((String)table.getValueAt(i, COUNT_COL));
            data.add(new MatchReplace((String)table.getValueAt(i, MATCH_COL), (String)table.getValueAt(i, REPLACE_COL), count));
        }
        return data;
    }
    ...
    public void setData(int index, int in_count) {
        System.out.println(("testing"));
        System.out.println((in_count));
        table.setValueAt(Integer.toString(in_count), index, COUNT_COL);
        System.out.println(("done"));
        System.out.println((String)table.getValueAt(index, COUNT_COL));
    }

    private void prepareTable() {
        table = new JTable();

        dataModel = new DefaultTableModel(new Object[][] {}, new String[] {"#", "Match", "Replace", "Count"}) {
    ...
    private void prepareEditButton(JPanel panel_1) {
        ...
        btnEdit.addActionListener((event) -> {
            ...
            JTextField count = new JTextField((String)table.getValueAt(selectedRow, COUNT_COL));
            Object[] message = {
                    "Match:", match,
                    "Replace:", replace,
                    "Count", count
            ...
            if (option == JOptionPane.OK_OPTION) {
                ...
                table.setValueAt(count.getText(), selectedRow, COUNT_COL);
            }
        });
        panel_1.add(btnEdit);
    }
    ...
    private void prepareAddButton(JPanel panel_1) {
        ...
        btnAdd.addActionListener((event) -> {
            ...
            if (option == JOptionPane.OK_OPTION) {
                ...
                dataModel.addRow(new Object[] { position, match.getText(), replace.getText(),count.getText() });
            }
        });
        panel_1.add(btnAdd);
    }
